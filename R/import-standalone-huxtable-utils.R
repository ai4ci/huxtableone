...
# Source: https://github.com/terminological/ggrrr/blob/HEAD/R/standalone-huxtable-utils.R
# Generated by: pkgtools::use_standalone("terminological/ggrrr", "huxtable-utils")
# ----------------------------------------------------------------------
#
# ---
...
.nested_arrange = function(tidyDf, groupVars) {
  ...
  colOrder = colOrder %>%
    ...
    dplyr::select(-.o, -.o2)
  if (length(.as_join_list(groupVars)) == 0) {
    return(tidyDf %>% dplyr::cross_join(colOrder))
  } else {
    return(
      tidyDf %>% dplyr::inner_join(colOrder, by = .as_join_list(groupVars))
    )
  }
}
