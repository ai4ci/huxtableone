...
.normality.tests = list(
  ...
  "cvm" = list(
    fun = .wrap(nortest::cvm.test),
    name = .name_of(nortest::cvm.test)
  ),
  "lillie" = list(
    fun = .wrap(nortest::lillie.test),
    name = .name_of(nortest::lillie.test)
  ),
  "pearson" = list(
    fun = .wrap(nortest::pearson.test),
    name = .name_of(nortest::pearson.test)
  ),
...
.col_type = function(col, max_levels) {
  ...
  if (is.character(tmp) & length(unique(tmp)) < max_levels) {
    return("categorical")
  }
  if (is.logical(tmp)) {
    return("categorical")
  }
  if (is.numeric(tmp) & length(unique(tmp)) < max_levels) {
    return("categorical")
  }
  if (is.numeric(tmp) & length(unique(tmp)) < max_levels) {
    return("ordered")
...
.p_is_normal = function(
  col,
  method = c("ad", "cvm", "lillie", "pearson", "sf"),
  trans = function(x) x
) {
  if (!is.numeric(col)) {
    return(NA)
...
any columns."
  ...
  if (length(not_matched) > 0) {
    stop(
      "Columns found in `cols` parameter but not present in dataframe: ",
  ...
  tmp = tibble::tibble(
    ...
    dplyr::mutate(
      .unit = lapply(.name, function(n) units[[n]]) %>%
      ...
      .p_is_normal = lapply(.content, .p_is_normal, method = normality_test) %>%
        unlist(),
    ...
    df_shape = df_shape %>%
      dplyr::mutate(
        ...
        .comparison_method = dplyr::case_when(
          ...
          .comparisons == 2 &
            .type == "continuous" &
            .is_normal >= normality_signif ~ "t-test",
          # continuous not normally distributed with ties
          .comparisons == 2 &
            .type == "continuous" &
            !.is_normal &
            .is_tied ~ "2-sided wilcoxon",
...
.describe_normality_test = function() {
  ...
  normality_signif = getOption("huxtableone.normality_significance", 0.005)
  sprintf(
    "Normal distributions determined by the %s (P>%1.1g)",
