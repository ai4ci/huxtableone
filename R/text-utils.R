...
# Set the default font option. Can be overridden as an option.
.default_font = function() {
  getOption("huxtableone.font", "Arial")
}
...
.pvalue.defaults = list(
  sampl = function(p) {
    dplyr::case_when(p < 0.001 ~ "<0.001", TRUE ~ .sprintf_na("%1.2g", p))
  },
  nejm = function(p) {
    dplyr::case_when(p < 0.001 ~ "<0.001", TRUE ~ .sprintf_na("%1.1g", p))
  },
  jama = function(p) {
    dplyr::case_when(
      p < 0.001 ~ "<0.001",
      p > 0.99 ~ ">0.99",
      TRUE ~ .sprintf_na("%1.2g", p)
    )
  },
  lancet = function(p) {
    dplyr::case_when(p < 0.0001 ~ "<0.0001", TRUE ~ .sprintf_na("%1.2g", p))
  },
  aim = function(p) {
    dplyr::case_when(
      p < 0.001 ~ "<0.001",
      p >= 0.001 & p < 0.2 ~ .sprintf_na("%1.3f", p),
      TRUE ~ .sprintf_na("%1.2f", p)
    )
  }
)
...
.adjust_fmt = function(fmt, percent = 1, real = 3) {
  ...
  fmt = dplyr::case_when(
    is.na(percent) ~ fmt,
    is.numeric(percent) ~ fmt %>%
      stringr::str_replace_all(
        "%([0-9]+)\\.([0-9]+)(f|g)%%",
        paste0("%\\1.", percent, "\\3%%")
      ),
    is.character(percent) ~ fmt %>%
      stringr::str_replace_all(
        "%([0-9]+)\\.([0-9]+)(f|g)%%",
        paste0("%\\1.", percent, "%%")
      ),
